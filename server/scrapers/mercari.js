...
async function search(query, strictEnabled = true, filters = []) {
        ...
        try {
            ...
            for (let pageNum = 1; pageNum <= MAX_PAGES; pageNum++) {
                ...
                if (nextButtonHref && pageNum < MAX_PAGES) {
                    ...
                    // href from getAttribute is relative or full? verify.
                    // Usually relative on SPA, but let's robustly handle it.
            ...
            console.error('Mercari Search Loop Error:', error);
        }
    ...
    const timeoutPromise = new Promise(resolve => {
        timeoutHandle = setTimeout(() => {
            console.warn(`Mercari: Search for "${query}" timed out after 4m.`);
            resolve('TIMEOUT');
        }, 240000);
    });
